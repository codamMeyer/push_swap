							...
							t_write_instruction write_instruction)
{
	const t_optional_index	element_index = \
							find_element_index(&(stacks->a), element);
	const t_bool			close_to_top = \
							is_close_to_top(stacks, element_index.index);
	int				num_moves;
	
	if (!element_index.initialized)
		return (0);
	if (close_to_top)
	{
		num_moves = stacks->a.top - element_index.index;
		execute_operation(stacks, num_moves, ra);
		write_instruction(STR_RA, num_moves);
	}
	else
	{
		num_moves = element_index.index + 1;
